...
module.exports = {
  ...
  getCodes () {
    ...
    for (const type of this.getIconTypes()) {
      ...
      this.addCodes(string, type, codes)
    }
    return ExtendJS.sortObjectByKey(codes)
  },

  addCodes (string, type, codes) {
    const variantType = this.getVariantType(type)
    for (const line of string.split('\n')) {
      if (!line.trim()) continue
      const value = line.trim().substring(0, line.indexOf(' '))
      if (!codes[value]) codes[value] = []
      codes[value].push(variantType)
    }
  },

  getVariantType (type) {
    const value = type.replace('Material Icons', '').trim().toLowerCase()
